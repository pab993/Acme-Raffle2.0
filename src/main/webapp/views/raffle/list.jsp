...
<security:authorize access="hasRole('MANAGER')">
	<div>
		<h3>
			<a href="raffle/create.do"> <spring:message code="raffle.create" /></a>
		</h3>
	</div>
</security:authorize>
...
<display:table name="raffles" pagesize="5" class="displaytag"
	...
	<security:authorize access="hasRole('USER')">
		<display:column>
			<jstl:if test="${row.publicationTime lt now and row.deadline gt now }">
	...
	<security:authorize access="hasRole('MANAGER')">
		<display:column>
			<jstl:if test="${row.publicationTime > now}">
				<a href="raffle/edit.do?raffleId=${row.id}"> <spring:message
						code="raffle.edit"></spring:message></a>
			</jstl:if>
		</display:column>
		...
		<display:column>
			<jstl:if test="${row.publicationTime > now}">
				<a href="prize/create.do?raffleId=${row.id}"> <spring:message
