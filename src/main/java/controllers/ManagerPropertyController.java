...
public class ManagerPropertyController extends AbstractController {
	...
	public ModelAndView addProperty(final int propertyId, final int prizeId, @RequestParam(defaultValue = "0", required = false) final int errorMessage) {
		...
		try {
			...
			try {
				...
				res.setViewName("redirect:/manager/property/listPrize.do?prizeId=" + prizeId);
			} catch (final Throwable oops) {
				res = new ModelAndView("redirect:/manager/property/listPrize.do?prizeId=" + prizeId + "&errorMessage=2");
			}
	...
	public ModelAndView deleteProperty(final int propertyId, final int prizeId, @RequestParam(defaultValue = "0", required = false) final int errorMessage) {
		...
		try {
			...
			try {
				...
				res = new ModelAndView("redirect:/manager/property/listPrize.do?prizeId=" + prizeId + "&errorMessage=3");
			}
			this.error(res, errorMessage);
